import { c as commonjsGlobal, S as SharedState, b as UPX_EXCHANGE_RATE, U as UplandApi, T as TAB_URL, C as ClientError, e as API_BASE_URL, t as time, n as neighbourhoodsWithin, p as propertiesWithRent } from "./property.js";
var lodashExports = {};
var lodash = {
  get exports() {
...
const State$2 = Object.assign(stateProxy, {
  on: (event, handler) => {
...
function markNeighbourhoods(eventId, hoods) {
...
const storeCurrentCoordinates = (url, state2) => {
  const coord = (prop2) => parseFloat(url.searchParams.get(prop2) || "0");
  state2.currentCoordinates = ["north", "south", "east", "west"].reduce(
    (coords, direction) => {
...
const storeSession = async (state2, monitor2) => {
  state2.session = await monitor2.getSession();
  if (state2.session)
    state2.api = new UplandApi(state2.session.auth_token);
};
const onCompletedHandler = (state2, monitor2) => ({ url }) => {
  if (url.indexOf("/api/map?") > -1) {
    storeCurrentCoordinates(new URL(url), state2);
  }
  if (url.indexOf("/api/authentication") > -1) {
    storeSession(state2, monitor2).catch((err) => console.warn("Error updating session:", err));
  }
};
...
const createClientMonitor = async function(tab, state2) {
  try {
  ...
  return {
    ...
    listen() {
      chrome.webRequest.onCompleted.addListener(
        onCompletedHandler(state2, this),
        { urls: [API_BASE_URL + "/*"] }
      );
      chrome.runtime.onMessage.addListener(function(msg, sender) {
        if (msg.action === "windowResize") {
          state2.screenDimensions = msg.data;
    ...
    async markNeighbourhoods(hoods) {
...
let monitor;
const instance = async (state2, tab) => {
  if (monitor)
    return monitor;
  ...
  monitor = await createClientMonitor(currentTab, state2);
    ...
    async (newCoords) => {
      time({ root: "worker", label: `trackViewportState ${JSON.stringify(newCoords)}` }, async (timerOpts) => {
        ...
        try {
          ...
          state2.viewableProperties = ((_a2 = state2.viewableNeighbourhoods) == null ? void 0 : _a2.length) === 1 ? await propertiesWithRent(
